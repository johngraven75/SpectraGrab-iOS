...
public sealed class MobileLiveCaptureService : IMobileLiveCaptureService
{
    ...
    public async Task<MobileCaptureResult> CaptureAsync(string url, IProgress<MobileCaptureProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await using (var output = new FileStream(temporaryPath, FileMode.CreateNew, FileAccess.Write, FileShare.Read, 128 * 1024, true))
            {
                ...
                {
                    bytesWritten = isHls
                        ? await CaptureHlsAsync(uri, output, stopwatch, outputPath, progress, cancellationToken)
                        : await CaptureDirectAsync(uri, output, stopwatch, outputPath, progress, cancellationToken);
                }
                catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { stopped = true; }
                finally
                {
            ...
            if (bytesWritten <= 0)
                throw new InvalidOperationException(stopped ? "Capture stopped before any media data was received; no file was created." : "The stream ended without producing media data; no file was created.");
    ...
    private async Task<long> CaptureHlsAsync(Uri manifestUri, Stream output, Stopwatch stopwatch, string outputPath, IProgress<MobileCaptureProgress>? progress, CancellationToken token)
    {
        ...
        while (true)
        {
            ...
            {
                var variant = lines.LastOrDefault(line => !line.StartsWith('#')) ?? throw new InvalidOperationException("HLS master playlist contains no playable variants.");
                manifestUri = new Uri(manifestUri, variant);
                continue;
            }
            if (lines.Any(line => line.StartsWith("#EXT-X-KEY", StringComparison.OrdinalIgnoreCase) && !line.Contains("METHOD=NONE", StringComparison.OrdinalIgnoreCase)))
    ...
    private async Task<long> CaptureDirectAsync(Uri uri, Stream output, Stopwatch stopwatch, string outputPath, IProgress<MobileCaptureProgress>? progress, CancellationToken token)
    {
    ...
    private static async Task<long> CopyToCaptureAsync(Stream input, Stream output, long existingBytes, Stopwatch stopwatch, string outputPath, IProgress<MobileCaptureProgress>? progress, CancellationToken token)
    {
    ...
    private static TimeSpan RefreshDelay(IReadOnlyList<string> lines)
    {
        ...
        return targetLine is not null && double.TryParse(targetLine[(targetLine.IndexOf(':') + 1)..], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var seconds)
            ? TimeSpan.FromSeconds(Math.Clamp(seconds / 2, 1, 6)) : TimeSpan.FromSeconds(2);
    }
    private static string MediaExtension(Uri uri) { var extension = Path.GetExtension(uri.AbsolutePath); return string.IsNullOrWhiteSpace(extension) || extension.Length > 8 ? ".mp4" : extension; }
    private static string UniquePath(string directory, string name, string extension) { var candidate = Path.Combine(directory, name + extension); for (var index = 1; File.Exists(candidate); index++) candidate = Path.Combine(directory, $"{name}-{index}{extension}"); return candidate; }
