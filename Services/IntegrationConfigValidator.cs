...
public static class IntegrationConfigValidator
{
    ...
    public static IntegrationConfig Normalize(IntegrationConfig defaults, IntegrationConfig current)
    {
        ...
        {
            throw new InvalidDataException($"Configuration {defaults.Id} is missing its display name.");
        }
        ...
        if (node is JsonObject obj)
        {
            foreach (var (key, value) in obj)
            {
                ...
                if (LooksSensitive(key)
                    ...
                    && !key.Contains("SecureStorageKey", StringComparison.OrdinalIgnoreCase)
    ...
    private static bool LooksSensitive(string key) => SensitiveNames.Any(name =>
    ...
    private static bool LooksLikeUrl(string key) =>
        key.Equals("endpoint", StringComparison.OrdinalIgnoreCase) || key.EndsWith("Url", StringComparison.OrdinalIgnoreCase);
}
