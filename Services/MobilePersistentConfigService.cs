...
public sealed class MobilePersistentConfigService : IMobilePersistentConfigService
{
    ...
    public async Task<IntegrationConfigReport> EnsureInitializedAsync()
    {
        ...
        try
        {
            ...
            providers = [];
            foreach (var defaults in providerDefaults)
            {
                providers.Add(await LoadAsync(providerDirectory, defaults, counters));
            }
            plugins = [];
            foreach (var defaults in pluginDefaults)
            {
                plugins.Add(await LoadAsync(pluginDirectory, defaults, counters));
            }
            ...
            Status = $"Verified {providers.Count} provider and {plugins.Count} add-in configs in app-scoped storage. "
    ...
    private static string Serialize(IntegrationConfig value) =>
