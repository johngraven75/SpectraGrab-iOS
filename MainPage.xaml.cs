...
public partial class MainPage : ContentPage
{
    ...
    public MainPage(IMobileAutomatedDownloadService automation, IMobileLiveCaptureService captureService, IMobilePersistentConfigService persistentConfigs)
    {
        InitializeComponent();
    ...
    private void OnInspectClicked(object? sender, EventArgs e) =>
        StatusLabel.Text = TryGetHttpUri(UrlEntry.Text, out var uri) ? $"Ready to inspect: {uri.Host}" : "Enter a valid HTTP or HTTPS URL.";
    ...
    private async void OnAutomatedDownloadClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            var progress = new Progress<double>(value => MainThread.BeginInvokeOnMainThread(() => DownloadProgress.Progress = Math.Clamp(value, 0, 100) / 100d));
            var result = await automation.DownloadAsync(uri.AbsoluteUri, progress, downloadCancellation.Token);
            StatusLabel.Text = result.Warnings.Count == 0 ? $"Complete: metadata and poster verified. Saved to {result.FilePath}" : $"Complete with warnings: {string.Join("; ", result.Warnings)}";
        }
    ...
    private async void OnCaptureClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            var progress = new Progress<MobileCaptureProgress>(update => MainThread.BeginInvokeOnMainThread(() => CaptureStatusLabel.Text = $"{update.Status} • {update.Elapsed:hh\\:mm\\:ss} • {FormatBytes(update.BytesWritten)}"));
            var result = await captureService.CaptureAsync(uri.AbsoluteUri, progress, captureCancellation.Token);
            CaptureStatusLabel.Text = result.StoppedByUser ? $"Stopped and finalized {FormatBytes(result.BytesWritten)} at {result.OutputPath}" : $"Capture complete: {FormatBytes(result.BytesWritten)} at {result.OutputPath}";
        }
        catch (Exception ex) { CaptureStatusLabel.Text = ex.Message; }
        finally
        {
    ...
    {
        if (Uri.TryCreate(value?.Trim(), UriKind.Absolute, out var parsed) && (parsed.Scheme == Uri.UriSchemeHttp || parsed.Scheme == Uri.UriSchemeHttps))
        {
            uri = parsed;
            return true;
        }
